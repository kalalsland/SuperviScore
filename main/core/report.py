...
def write_all(teachers: list[Teacher], profile: dict, out_dir: str):
    """teachers 已按推荐分倒序。写 CSV(全部) + Top N 详情。"""
    ...
    for i, t in enumerate(teachers[:top_n], 1):
        try:
            p = write_detail(t, i, profile, detail_dir, gen_letter=True)
            log(f"[report] 详情+套磁信 第{i}名 {t.name} -> {os.path.basename(p)}")
        except Exception as e:
            log(f"[report] 详情写出失败 {t.name}: {e}")
    log(f"[report] Top {top_n} 详情已写入: {detail_dir}")
